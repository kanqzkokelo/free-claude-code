...
FIELDS: tuple[ConfigFieldSpec, ...] = (
    ...
    ConfigFieldSpec(
        ...
        "Antigravity Model",
        "providers",
        settings_attr="antigravity_model",
        default="default",
        description=(
            "Google Cloud Code Antigravity model id. Use ``default`` to let the backend "
            "choose, or specify e.g. ``gemini-3-flash``. Run ``fcc-antigravity-login`` first."
        ),
    ),
    ConfigFieldSpec(
        "ANTIGRAVITY_PROXY",
        ...
        "providers",
        "secret",
        settings_attr="antigravity_proxy",
        secret=True,
        advanced=True,
    ),
    ConfigFieldSpec(
...
def provider_config_status(
    ...
    for provider_id, descriptor in PROVIDER_CATALOG.items():
        if descriptor.credential_env is None:
            # Static-credential providers (e.g. antigravity OAuth) — check for
            # persisted credentials on disk so the card shows a useful status.
            if descriptor.static_credential is not None:
                token_file = _oauth_token_path_for(descriptor)
                if token_file is not None:
                    configured = token_file.is_file()
                    statuses.append(
                        {
                            "provider_id": provider_id,
                            "kind": "oauth",
                                                       "status": "configured" if configured else "missing_key",
                            "label": "Configured" if configured else "Run fcc-antigravity-login",
                            "credential_env": "OAuth tokens",
                        }
                    )
                    continue
                statuses.append(
                    {
                        ...
                        "credential_env": "Static credential",
                    }
                )
                continue
            base_url = ""
...
def _oauth_token_path_for(descriptor: Any) -> Path | None:
    """Return the persisted OAuth token path for a descriptor, if applicable."""
