...
def _create_antigravity(config: ProviderConfig, settings: Settings) -> BaseProvider:
    from providers.antigravity import AntigravityProvider

    return AntigravityProvider(config, model=settings.antigravity_model)
...
def _create_unlimited_ai(config: ProviderConfig, _settings: Settings) -> BaseProvider:
    from providers.unlimited_ai import UnlimitedAIProvider

    return UnlimitedAIProvider(config)
...
PROVIDER_FACTORIES: dict[str, ProviderFactory] = {
    ...
    "unlimited_ai": _create_unlimited_ai,
    "wafer": _create_wafer,
    "kimi": _create_kimi,
...
def _model_list_provider_ids_for_settings(settings: Settings) -> tuple[str, ...]:
    ...
    for provider_id, descriptor in PROVIDER_DESCRIPTORS.items():
        if descriptor.static_credential is not None:
            # Static-credential providers (e.g. antigravity OAuth) are always
            # eligible for model discovery — their auth is handled internally.
